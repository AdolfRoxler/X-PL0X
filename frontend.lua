...
local backend = devmode and loadstring(game:HttpGet'https://raw.githubusercontent.com/AdolfRoxler/X-PL0X/dev/libraries/X-API.lua')() or loadstring(game:HttpGet'https://raw.githubusercontent.com/AdolfRoxler/X-PL0X/main/libraries/X-API.lua')()
		...
		for _,N in pairs(PLAYERLIST) do
			...
			if Char and Box and Arrow[10] and Healthbar[3] and NameTag and HeadE and (Char:GetModelCFrame().p-Camera.CFrame.p).Magnitude<=25000 then --- hard limit lol
				...
				if SKELETONS[_] then
					for s,S in pairs(SKELETONS[_]) do
						...
						if one and two and Config.Skeleton==true then
							...
							two.Visible = false--_:IsAncestorOf(Camera.CameraSubject.Character)==false and 
				...
				if Config.TRACERST==true then
					...
					Tracer.ZIndex = boxzindex+4 if TT.Z<0 then TT=math:InverseWorldToViewportPoint(Pos*Ve3n(0,sy2,0)) end Tracer.To = Ve2n(TT.X,TT.Y) end
